...
We use a large weight on the cost relates to the change of delta to ensure a smooth change of steering angle. The target speed is set for 100mph.
...
In order to account the latency, we use the solution from the solver at a t=0.1s delay (`MPC.cpp` line #238-241).
